        ...
        <Col span={6}>
          ...
          >
            {organizadores.carrinhos.map(org => (
              ...
              <Option key={org.numero_suborganizador} value={org.numero_suborganizador}>
                {org.numero_suborganizador}
              </Option>
                ...
                .flatMap(org => 
                  Array.isArray(org.sub_organizadores) && org.sub_organizadores.length > 0
                    ? org.sub_organizadores.map(sub => (
                        <Panel header={`Gaveta ${sub.numero_suborganizador}`} key={sub.numero_suborganizador}>
            ...
            <Collapse style={{ marginTop: '10px' }}>
              {organizadores.armarios
                ...
                .flatMap(org => 
                  Array.isArray(org.sub_organizadores) && org.sub_organizadores.length > 0
                    ? org.sub_organizadores.map(sub => (
                        <Panel header={`Prateleira ${sub.numero_suborganizador}`} key={sub.numero_suborganizador}>
                          <p>{sub.detalhes || 'Sem detalhes disponíveis.'}</p>
                        </Panel>
                      ))
